        ...
        <div class="status-info">
          ...
          @if (healthReport()?.totalDuration) {
            <span class="status-duration"
              >Response time: {{ healthReport()?.totalDuration | number: '1.0-0' }}ms</span
            >
          }
        </div>
...
export class HealthComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.refreshTrigger$.pipe(takeUntil(this.destroy$)).subscribe(() => this.fetchHealth());
  }
  ...
  private fetchHealth(): void {
    ...
    this.http
      .get<HealthReport>('/api/health/detailed')
      .pipe(
        catchError((err) => {
          if (err.status === 0) {
